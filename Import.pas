...
Function SY_GetFont(Height: Double, Angle: TRotationBy90, FontName: String, Bold: Boolean, Italic: Boolean, Underline: Boolean): TFontID;
Var
    sz : Integer;
Begin
    sz := (Height * 0.1);
    ...
    Result := SchServer.FontManager.GetFontID(sz, Angle, Underline, Italic, Bold, False, FontName);
End;
...
Begin
    ...
    End;
    s1 := GetBetween(Data, '(FillColor ', ')');
    If s1 <> '' Then Begin
        rect.AreaColor := StrToInt('$' + s1);
...
Begin
    ...
    txt.FontID := SY_GetFont(Evaluate(GetBetween(Data, 'Height ', ')')), txt.Orientation, GetBetween(Data, '(Font "', '")'), GetBetween(Data, '(Bold ', ')') = 'True', GetBetween(Data, '(Italic ', ')') = 'True', GetBetween(Data, '(Underline ', ')') = 'True');
...
Begin
    ...
    prm.IsHidden := True;
    If GetBetween(Data, '(Visible ', ')') = 'True' Then Begin
        prm.IsHidden := False;
    End;
    ...
    prm.FontID := SY_GetFont(Evaluate(GetBetween(Data, 'Height ', ')')), prm.Orientation, GetBetween(Data, '(Font "', '")'), GetBetween(Data, '(Bold ', ')') = 'True', GetBetween(Data, '(Italic ', ')') = 'True', GetBetween(Data, '(Underline ', ')') = 'True');
...
Begin
    ...
    End;
    // Symbol Library
    SchLibDoc := Client.OpenNewDocument(cDocKind_SchLib, 'Components', 'Components', False);
...
Begin
    ...
    If CheckLeft(GetCurrentProductBuild, '19.') Then Begin
        ShowMessage('NOTE: Altium 19 has issues with the Schematic FontManger''s scripting interface.' +
        '  All symbol texts and parameters will use the FontID specified by the ForceSchFontID integer at the top of Import.pas.');
        BrokenSCHFontManager := 1;
    End;
    // 2019-01-16 gbn end

    //SavePath := LeftOf(InFileName, '.'); // 2019-10-02 gbn
    TxtDir   := GetFileLocation(InFileName);
    LibsBase := GetFileLocation(LeftOfLast(TxtDir, '\'));
    ForceDirectories(LibsBase + 'Libs');
    SavePath := LibsBase + 'Libs\' + RightOf(LeftOfLast(InFileName, '.'), TxtDir);
...
Procedure ImportAllAscIIData(FileNames: TStrings, LibraryName: String);
Var
    WorkSpace : IWorkSpace;
    Proj : IProject;
    ProjDoc : IServerDocument;
    ...
    pLib : IPCB_Library;
    sLib : ISch_Document;
    DefFP : IPCB_Component;
    DefSY : ISch_Component;
    TxtDir, LibsBase, SavePath : String;
    InFile : TextFile;
    Errors : TStringList;
    inp, tag : String;
    i : Integer;
Begin
    ...
    TxtDir   := GetFileLocation(FileNames[0]);
    LibsBase := GetFileLocation(LeftOfLast(TxtDir, '\'));
    ForceDirectories(LibsBase + 'Libs');
    SavePath := LibsBase + 'Libs\' + LibraryName;
    Errors := TStringList.Create();
    ...
    For i := 0 To FileNames.Count - 1 Do Begin
        ...
        While Not EOF(InFile) Do Begin
            ...
            tag := Trim(tag);
            Case tag Of
            'StartFootprints': ImportFootprints(InFile, pLib, Errors, FileNames[i]);
            'StartComponents': ImportComponents(InFile, sLib, Errors);
            '': Continue;
            End;
        End;
        CloseFile(InFile);
